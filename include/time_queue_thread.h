...
namespace small {
    ...
    public:
        ...
        inline void start_threads()
        {
            if (!m_thread_future.valid()) {
                m_thread_future = std::async(std::launch::async, &time_queue_thread::thread_function_delayed, this);
            }
        }
        ...
        {
            m_time_queue.signal_exit_when_done();
            auto status = m_thread_future.wait_until(__atime);
    ...
    private:
        ...
        // members
        //
        time_queue<T>     m_time_queue;    // a time priority queue for delayed items
        ActiveQueueT     &m_active_queue;  // active queue where to push
        std::future<void> m_thread_future; // thread delayed futures (needed to wait for)
    };
} // namespace small
